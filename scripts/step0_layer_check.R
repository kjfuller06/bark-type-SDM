...
c = codes %>% 
  filter(code == 4326) %>% 
  dplyr::select(prj4)
grids <- list.files("./processed datasets/PlanCurvature/NSW" , pattern = "hdr.adf", recursive = TRUE, full.names = TRUE)
# load borders of raster files
plan1 = st_bbox(raster(grids[1])) %>% st_as_sfc() %>% st_as_sf()
plan2 = st_bbox(raster(grids[2])) %>% st_as_sfc() %>% st_as_sf()
sfplan = rbind(plan1, plan2)
for(i in c(3:length(grids))){
  x = raster(grids[i])
  x = st_bbox(x) %>% st_as_sfc() %>% st_as_sf()
  sfplan = rbind(sfplan, x)
}
st_crs(sfplan) = 4326
...
grids140 = grids[4:7]
sfplan3 = sfplan %>% 
  filter(files %in% grids140)
sfplan = rbind(sfplan2, sfplan3)
grids = unique(sfplan$files)
# plot
tmap_mode("view")
tm_shape(nsw)+tm_borders()+tm_shape(sfplan)+tm_borders()
...
# extract file names
grids <- list.files("./processed datasets/ProfileCurvature/NSW" , pattern = "hdr.adf", recursive = TRUE, full.names = TRUE)
# load borders of raster files
plan1 = st_bbox(raster(grids[1])) %>% st_as_sfc() %>% st_as_sf()
plan2 = st_bbox(raster(grids[2])) %>% st_as_sfc() %>% st_as_sf()
sfplan = rbind(plan1, plan2)
...
# extract file names
grids <- list.files("./processed datasets/TWI/NSW" , pattern = "hdr.adf", recursive = TRUE, full.names = TRUE)
# load borders of raster files
plan1 = st_bbox(raster(grids[1])) %>% st_as_sfc() %>% st_as_sf()
plan2 = st_bbox(raster(grids[2])) %>% st_as_sfc() %>% st_as_sf()
sfplan = rbind(plan1, plan2)
...
# extract file names
grids <- list.files("./processed datasets/SRADTotalShortwave/NSW/" , pattern = "0115.flt", recursive = TRUE, full.names = TRUE)
# load borders of raster files
date = data.frame(year = 2000,
                  month = c(1:12),
                  day = c(15, 14, rep(15, 10)))
date$date <- apply( date[ , c(1:3) ] , 1 , paste , collapse = "-" )
date = date[,4]
sradtot1 = st_bbox(raster(grids[1])) %>% st_as_sfc() %>% st_as_sf()
sradtot2 = st_bbox(raster(grids[2])) %>% st_as_sfc() %>% st_as_sf()
sfsradtot = rbind(sradtot1, sradtot2)
for(i in c(3:length(grids))){
  x = raster(grids[i])
  x = st_bbox(x) %>% st_as_sfc() %>% st_as_sf()
  sfsradtot = rbind(sfsradtot, x)
}
st_crs(sfsradtot) = 4326

# select NSW tiles
sfsradtot$files = grids
sfsradtot2 = sfsradtot[nsw,]
grids140 = grids[3:7]
sfsradtot3 = sfsradtot %>% 
  filter(files %in% grids140)
sfsradtot = rbind(sfsradtot2, sfsradtot3)
grids = unique(sfsradtot$files)
# plot
tmap_mode("view")
tm_shape(nsw)+tm_borders()+tm_shape(sfsradtot)+tm_borders()
...
# extract file names
grids <- list.files("./processed datasets/SRADShortwaveRatio/SRADRat_NSW/" , pattern = "0115.flt", recursive = TRUE, full.names = TRUE)
# load borders of raster files
sradrat1 = st_bbox(raster(grids[1])) %>% st_as_sfc() %>% st_as_sf()
sradrat2 = st_bbox(raster(grids[2])) %>% st_as_sfc() %>% st_as_sf()
sfsradrat = rbind(sradrat1, sradrat2)
for(i in c(3:length(grids))){
  x = raster(grids[i])
  x = st_bbox(x) %>% st_as_sfc() %>% st_as_sf()
  sfsradrat = rbind(sfsradrat, x)
}
st_crs(sfsradrat) = 4326

# select NSW tiles
sfsradrat$files = grids
sfsradrat2 = sfsradrat[nsw,]
grids140 = grids[3:7]
sfsradrat3 = sfsradrat %>% 
  filter(files %in% grids140)
sfsradrat = rbind(sfsradrat2, sfsradrat3)
grids = unique(sfsradrat$files)
# plot
tmap_mode("view")
tm_shape(nsw)+tm_borders()+tm_shape(sfsradrat)+tm_borders()
